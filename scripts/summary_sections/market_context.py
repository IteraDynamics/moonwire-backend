...
def append(md: List[str], ctx: SummaryContext) -> None:
    artifact_dirs = [Path("artifacts"), ctx.logs_dir.parent / "artifacts", ctx.models_dir.parent / "artifacts"]
    seen, arts = set(), []
    for d in artifact_dirs:
        r = d.resolve()
        if r not in seen:
            seen.add(r); arts.append(d)

    ensure_dir(ctx.logs_dir); ensure_dir(ctx.models_dir)
    for d in arts: ensure_dir(d)
    ...
    for cid in coins:
        ...
        r = rets.get(cid, {})
        h1, h24, h72 = r.get(\"h1\", 0.0), r.get(\"h24\", 0.0), r.get(\"h72\", 0.0)
