...
Outputs
 ...
 - artifacts/consensus_social.png        (1280x720 social image for #BuildInPublic)
...
SOCIAL_W, SOCIAL_H, DPI = 1280, 720, 110  # 16:9 for social
# ------------------------------------------------
...
def weight_to_label(w: float) -> str:
    # snap to nearest band and label
    if w >= 1.125: return "High"
    if w >= 0.875: return "Med"
    return "Low"

def parse_ts(val):
    """Accept float epoch seconds or ISO8601 strings; return aware datetime UTC."""
    if val is None:
        return None
    try:
        # float/ints
        ...
        pass
    try:
        # lazy ISO parser without extra deps
...
def generate_demo_data_if_needed(reviewers, flag_times=None):
    """
    If DEMO_MODE=true and reviewers list is empty,
    seed 3-5 demo reviewers with weights in {0.75, 1.0, 1.25} and timestamps within last 60m.
    Returns (reviewers, seeded_events)
    NOTE: tests may call with one arg; flag_times is optional.
    """
...
for r in sorted(sig_rows, key=lambda x: x.get("timestamp", 0)):
    # collect raw timestamps for sparkline
    t = parse_ts(r.get("timestamp"))
    if t:
        flag_times.append(t)

    rid = r.get("reviewer_id", "")
    if rid in seen:  # first flag counts toward consensus display
        continue
    seen.add(rid)
...
# ---- If demo: seed if empty (no writes to disk) ----
...
def draw_social_card():
    # Colors — dark UI with bright accents for Twitter
    bg      = "#0B0F19"
    panel   = "#101826"
    text    = "#E6EEF8"
    subtext = "#9FB3C8"
    ...
    fig.text(0.06, 0.90, title, color=text, fontsize=34, weight=700, ha="left", va="center", transform=F)
    ...
    # ---- Consensus gauge (center-left)
    gauge_x, gauge_y, gauge_w, gauge_h = card_x + 0.03, card_y + 0.37, 0.50, 0.14
    # track
    fig.patches.append(FancyBboxPatch((gauge_x, gauge_y), gauge_w, gauge_h,
                                      boxstyle="round,pad=0.015,rounding_size=0.012",
                                      linewidth=0, facecolor=grid, transform=F))
    # fill
    cap = max(threshold, total_weight, 3.0)
    fill_w = max(0.0, min(gauge_w, (total_weight / cap) * gauge_w))
    if fill_w > 0:
        fig.patches.append(FancyBboxPatch((gauge_x, gauge_y), fill_w, gauge_h,
                                          boxstyle="round,pad=0.015,rounding_size=0.012",
                                          linewidth=0, facecolor=accent, transform=F))
    # threshold marker
    th_x = gauge_x + (threshold / cap) * gauge_w
    fig.patches.append(Rectangle((th_x-0.003, gauge_y-0.02), 0.006, gauge_h+0.04,
                                 linewidth=0, facecolor=th_col, transform=F))
    # labels (large, tweet-friendly)
    fig.text(gauge_x, gauge_y + gauge_h + 0.035, "Consensus vs Threshold", color=subtext,
             fontsize=16, ha="left", va="bottom", transform=F)
    fig.text(gauge_x + fill_w + 0.005, gauge_y + gauge_h/2, f"{total_weight:.2f}",
             color=accent, fontsize=28, weight=700, ha="left", va="center", transform=F)
    fig.text(th_x, gauge_y + gauge_h + 0.005, f"{threshold:.2f}", color=th_col,
             fontsize=18, weight=600, ha="center", va="bottom", transform=F)

    # ---- Reviewer list (right)
    side_x, side_y, side_w, side_h = gauge_x + gauge_w + 0.04, gauge_y - 0.02, 0.33, gauge_h + 0.10
    fig.patches.append(FancyBboxPatch((side_x, side_y), side_w, side_h,
                                      boxstyle="round,pad=0.015,rounding_size=0.012",
                                      linewidth=0, facecolor=grid, transform=F))
    fig.text(side_x + 0.02, side_y + side_h - 0.04, "Reviewers (redacted)",
             color=subtext, fontsize=15, ha="left", va="top", transform=F)

    if reviewers:
        rows = []
        for r in reviewers[:8]:
            rows.append(f"• {red(r['id'])}  —  {weight_to_label(r['weight'])}")
        if len(reviewers) > 8:
            rows.append(f"… +{len(reviewers)-8} more")
        fig.text(side_x + 0.02, side_y + side_h - 0.08, "\n".join(rows),
                 color=text, fontsize=16, ha="left", va="top", transform=F)
    else:
        fig.text(side_x + 0.02, side_y + side_h - 0.08, "No reviewers yet",
                 color=text, fontsize=16, ha="left", va="top", transform=F)

    # ---- Flag timeline (bottom of card)
    tl_x, tl_y, tl_w, tl_h = card_x + 0.03, card_y + 0.16, card_w - 0.06, 0.10
    fig.patches.append(FancyBboxPatch((tl_x, tl_y), tl_w, tl_h,
                                      boxstyle="round,pad=0.015,rounding_size=0.012",
                                      linewidth=0, facecolor=grid, transform=F))
    fig.text(tl_x + 0.02, tl_y + tl_h + 0.01, "Flag timeline (last run)",
             color=subtext, fontsize=15, ha="left", va="bottom", transform=F)
    ...
    if flag_times:
        ...
        fig.patches.append(Rectangle((tl_x + 0.02, base_y), tl_w - 0.04, 0.006, linewidth=0, facecolor=panel, transform=F))
        # ticks
        for t in times:
            x01 = (t - t0).total_seconds() / span
            x = tl_x + 0.02 + x01 * (tl_w - 0.04)
            fig.patches.append(Rectangle((x-0.003, base_y-0.015), 0.006, 0.03, linewidth=0, facecolor=accent, transform=F))
    else:
        fig.text(tl_x + 0.02, tl_y + tl_h/2, "No flags this run", color=subtext, fontsize=14,
                 ha="left", va="center", transform=F)

    # footer
    fig.text(0.06, 0.07, f"moonwire • demo mode • {now}", color=subtext, fontsize=13, transform=F)
...
# ---- markdown summary (kept minimal + reliable) ----
md = []
